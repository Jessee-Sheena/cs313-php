...
?>
  ...
  if(preg_match("!image!", $_FILES['recipeImage']['type'])) {
    ...
    if (copy($_FILES['recipeImage']['tmp_name'], $recipeImage_path)) {
         ...
         $db->query("INSERT INTO recipe (recipe_name, recipe_description, cook_time, prep_time, cuisine, total_time, serving_size, calories, image, user) VALUES ('".$name."', '".$description. "', '".$cookTime."', '".$prepTime."', '".$cuisine."', '".$totalTime."', '".$serving."', '".$calories."', '".$recipeImage_path."', '" . $_SESSION['user']. "');");
